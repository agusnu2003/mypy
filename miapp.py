...
def procesar_datos(archivo_principal, archivo_generadores):
    ...
    # Verificar columnas en archivo principal
    ...
    # Verificar columnas en archivo de generadores
    ...
    # Actualizar 'Id de referencia' solo si está vacío
    df_principal['Id de referencia'] = df_principal.apply(
        lambda row: generadores_dict.get(row['Id de referencia'], row['Id de referencia']) if pd.isna(row['Id de referencia']) else row['Id de referencia'],
        axis=1
    )

    # Eliminar duplicados basados en columnas específicas
    df_principal = df_principal.drop_duplicates(subset=['Id de referencia', 'Fecha Recolección', 'Hora Recolección', 'cantidad', 'peso (kg)'])

    # Agregar columna 'Título' como columna vacía o con un valor predeterminado
    df_principal['nombre'] = df_principal['Título']

    # Seleccionar las columnas necesarias
    df_final = df_principal[['nombre', 'Id de referencia', 'Fecha Recolección', 'Hora Recolección', 'Observaciones', 'cantidad', 'peso (kg)']]
